...
`projects/` – Local projects, persisted in **MySQL via Prisma**. Contains `repository.js`, `service.js`, `import-service.js`, `import-worker.js` (BullMQ worker, started from `instrumentation.js`), `logs-service.js`, and `schemas.js`. Imports are processed asynchronously through the **BullMQ + Redis** queue (`lib/queue.js`): the upload enqueues a job and the in-process worker runs extraction → DAAIT MT (`/content/pecat`, with the project profile) → MTQE scoring (document turns READY here) → LLM review (`/content/post_edit`: verdict OK auto-approves, otherwise an applicable suggestion is stored on the TU). Stage constants and the pipeline vocabulary live in `modules/documents/pipeline-constants.js`; the stage jobs in `modules/documents/pipeline-service.js`.

`documents/` – Full document lifecycle for every non-SDLXLIFF format, handled **natively in this project**: `lib/tikal.js` (Okapi Tikal CLI, extraction with SRX segmentation from `okapi/srx/pdocs.srx` + merge, custom filter params in `okapi/filters/`) and `lib/soffice.js` (LibreOffice headless, PDF → docx). The SRX segments by sentence and also breaks clauses after `;` and `:` (guarded so times, ratios and `://` never split); the same file is shared byte-for-byte with module-pdocs and module-pangeadocs — keep the three in sync. Each document gets a working folder `storage/{documentId}/` (original + bilingual XLIFF — the STRUCTURAL template the export merges from; **it never changes after extraction**: on each export the reviewed targets come from MySQL and are written into a throwaway copy `storage/{documentId}/.merge/`, where the pre-merge fill and `tikal -m` run — a download must never mutate the working folder); `Project.documentId` links a project to it. `xliff.js` exposes one segment per sentence (`id = <transUnit>::<mid>`) with inline-code placeholders (`<g1>…</g1>`, `<x2/>`, `<b1/>`, `<e1/>`) that must reach the translated target unchanged. In the TU editor those placeholders render as atomic chips (`components/shared/inline-tags.jsx`); segments carrying them are edited with `components/TagEditor` (contenteditable that blocks deleting/reordering tags — Quill would destroy them by parsing the value as HTML), the rest keep Quill + LanguageTool. Also owns the shared-download flow (share uuid + `/api/file`). Requires Java + Okapi (`TIKAL_BIN`) and LibreOffice (`SOFFICE_BIN`, PDFs only) on the host — both preinstalled in the Docker image.
