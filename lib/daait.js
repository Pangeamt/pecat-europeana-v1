...
function getErrorMessage(error, fallback) {
  ...
  // FastAPI validation errors carry `detail` as an array/object; stringify
  // anything non-string so the message never logs as "[object Object]".
  const candidate =
    data?.detail ?? data?.message ?? data?.error ?? data ?? error?.message;
 ...
 *
 * tm_threshold and tm_mode are deliberately not sent: DAAIT ignores tm_mode
 * and tm_threshold only sets the `tm_match` flag of the response (default
 * 1.0 = exact match, the only case the import locks). Which memories the LLM
 * sees is decided by DAAIT's own profile threshold, and tm_info comes back
 * unfiltered.
 */
 ...
 * glossary_ids is opt-in (empty = none). task_level is never sent: DAAIT
 * applies the profile's (MEDIUM by default, or the one its preset fixes).
 */
export async function postEditContent({
  ...
  user_id,
  last_batch = false,
  // Required when the profile is language-agnostic (und/und): DAAIT then
  // needs the pair on every post_edit request.
  source_language,
  target_language,
}) {
  try {
...
/**
 * DAAIT quality presets (GET /llm/presets): [{ name, description, active,
 * available, unavailable_reason, roles: [{ role, model_name, status }],
 * grouping, task_level, draft_mode }]. A profile only stores the name.
 */
export async function listLlmPresets() {
  try {
    const response = await daaitClient.get("/llm/presets", {
      timeout: DAAIT_LIST_TIMEOUT_MS,
    });
    return Array.isArray(response.data) ? response.data : [];
  } catch (error) {
    mapAndThrow(error, "Failed to list DAAIT presets");
  }
}
...
export async function createProfile({
  ...
  try {
    ...
    // task_level is never sent: DAAIT defaults to MEDIUM, or takes the level
    // the preset fixes (a preset's level/draft_mode win when the profile is
    // saved). Only the preset NAME travels.
    if (llm_preset) payload.llm_preset = llm_preset;
...
export async function updateProfile({
  ...
  try {
    ...
    // An explicit null removes the preset from the profile in DAAIT.
    if (llm_preset !== undefined) payload.llm_preset = llm_preset;
...
export async function getProfile(id, { timeout } = {}) {
  try {
    const response = await daaitClient.get(
    ...
    mapAndThrow(error, "Failed to get DAAIT profile");
  }
}
