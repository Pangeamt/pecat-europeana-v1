...
export const checkFile = (file) => {
  ...
  const extension = file.name
    ...
    .toLowerCase();
  const allowedExtensions = ALLOWED_FILE_EXTENSIONS.map((ext) =>
    ext.toLowerCase(),
  );
...
export const postMTQE = async (pairs) => {
  try {
    console.log("MTQE_HOST", MTQE_HOST);
    console.log("pairs", pairs);
    const response = await axios.post(MTQE_HOST, pairs, {
    ...
    throw new Error(`Error postMTQE: ${detail}`);
  }
};
