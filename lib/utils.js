...
export const oxygenTranslateFile = async ( {filePath, src_lang, tgt_lang, mt }) => {
  if (typeof window === "undefined") {
    try {
      ...
      const { data } = await axios.post( `${OXIGEN_API_HOST}translate_file`, form, { headers: { ...form.getHeaders(), accept: "application/json" } });
...
export const postMTQE = async (pairs) => {
  try {
    const response = await axios.post(
      `${MTQE_HOST}/module/mtqe_score/process_list`,
      pairs, 
      {
