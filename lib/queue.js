...
// Each MTQE runs on its own queue, separate from the import pipeline. The
// dedicated workers score with concurrency 1 (the MTQE service dislikes
// parallel scoring) while extraction/MT of other documents keeps flowing:
//   mtqe-v1 -> pipeline-score jobs (the stage that turns a document READY)
//   mtqe-v2 -> score-mtqe-v2 jobs (informational second score)
export const MTQE_V1_QUEUE = "mtqe-v1";
...
function getMtqeV1Queue() {
  if (!global.mtqeV1Queue) {
    global.mtqeV1Queue = new Queue(MTQE_V1_QUEUE, {
      connection: buildConnection(),
      defaultJobOptions: DEFAULT_JOB_OPTIONS,
    });
  }
  return global.mtqeV1Queue;
}
...
// Queue registry for the SUPER-only monitor (modules/queues): name -> getter.
const QUEUE_GETTERS = {
  [PROJECT_IMPORT_QUEUE]: getQueue,
  [MTQE_V1_QUEUE]: getMtqeV1Queue,
  [MTQE_V2_QUEUE]: getMtqeV2Queue,
};

export const MONITORED_QUEUE_NAMES = Object.keys(QUEUE_GETTERS);
...
/**
 * Adds a job to the MTQE v1 queue (the pipeline's scoring stage). Same
 * fail-fast semantics as enqueueProjectImport: the caller decides how to
 * degrade (chainScoreStage releases the document as READY).
 */
export async function enqueueMtqeV1(name, data) {
  const timeout = new Promise((_, reject) =>
    setTimeout(
      () => reject(new Error("enqueue timed out")),
      5_000,
  ...
  try {
    return await Promise.race([getMtqeV1Queue().add(name, data), timeout]);
  } catch {
    throw new HttpError(
      503,
      "Job queue unavailable (is Redis running?). The scoring stage was not scheduled.",
      "QUEUE_UNAVAILABLE",
    );
  }
}
...
/**
 * Starts the MTQE workers. Concurrency 1 on purpose: the MTQE service
 * struggles with parallel scoring (see the reference client), and these
 * queues must never steal throughput from the import pipeline.
 */
export function startMtqeV1Worker({ handlers, onFinalFailure, concurrency = 1 }) {
  return startWorker({
    queueName: MTQE_V1_QUEUE,
    globalKey: "mtqeV1Worker",
    handlers,
    onFinalFailure,
    concurrency,
  });
