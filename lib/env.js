...
const schema = Joi.object({
  ...
  HOST_OPENSEARCH: Joi.string().required(),
  HOST_OPENSEARCH_AUTH: Joi.string()
    .pattern(/^[^:]+:.+$/)
    .required()
    .messages({
      "string.pattern.base":
        "HOST_OPENSEARCH_AUTH must have the format user:password",
    }),
...
export const env = {
  ...
  HOST_OPENSEARCH: value.HOST_OPENSEARCH,
  HOST_OPENSEARCH_AUTH: value.HOST_OPENSEARCH_AUTH,
