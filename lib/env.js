...
const schema = Joi.object({
  ...
  // QE v1: combined-score-with-references sync endpoint (full URL) +
  // p-api-key. Legacy MTQE/MTQE_API_KEY still accepted as fallback.
  MTQE_V1: Joi.string().uri().optional().allow(""),
  MTQE_V1_API_KEY: Joi.string().optional().allow(""),
  MTQE: Joi.string().uri().optional().allow(""),
  MTQE_API_KEY: Joi.string().optional().allow(""),
  // QE v2: /score-with-references sync endpoint (full URL, one segment per
  // request) + X-API-Key. Unset = the pipeline skips the second pass.
  MTQE_V2: Joi.string().uri().optional().allow(""),
  MTQE_V2_API_KEY: Joi.string().optional().allow(""),
}).unknown(true);
...
export const env = {
  ...
  MTQE_V1: value.MTQE_V1,
  MTQE_V1_API_KEY: value.MTQE_V1_API_KEY,
  MTQE: value.MTQE,
  MTQE_API_KEY: value.MTQE_API_KEY,
  MTQE_V2: value.MTQE_V2,
  MTQE_V2_API_KEY: value.MTQE_V2_API_KEY,
};
